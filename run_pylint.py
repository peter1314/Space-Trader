...
def main(root=None, verbose=False, process_count=None, strict=False):
    ...
    newfiles = list()
    for pyfile in files:
        if ("/app/routes.py" in pyfile
        or "/app/py/player.py" in pyfile
        or "/app/__init__.py" in pyfile
        or "/app/py/__init__.py" in pyfile
        or "/app/py/region.py" in pyfile
        or "/app/py/trader.py" in pyfile
        or "/app/py/police.py" in pyfile
        or "/app/py/bandit.py" in pyfile
        or "/app/py/ship.py" in pyfile):
            fileinfo = 0
        else: 
            newfiles.append(pyfile)
        # print(myfile)
    files = newfiles
